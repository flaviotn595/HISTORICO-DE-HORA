...
function AppContent() {
  ...
  const handleDeleteEmployee = async (empId: number, empName: string) => {
    if (!window.confirm(`Tem certeza que deseja excluir o funcionário ${empName}?`)) return;

    setIsLoading(true);
    try {
      const success = await api.deleteEmployee(empId);
      if (success) {
        setEmployees(prev => prev.filter(e => e.id !== empId));
        // Remove from derived lists automatically via useMemo
      } else {
  ...
  const { houseEmployees, trinoEmployees } = useMemo(() => {
    const house: Employee[] = [];
    const trino: Employee[] = [];
    filteredEmployees.forEach(e => {
      if (e.name.toUpperCase().includes('TRINO')) trino.push(e);
      else house.push(e);
    });
    return { houseEmployees: house, trinoEmployees: trino };
  }, [filteredEmployees]);
      ...
      <div className="w-full p-2 md:p-6 lg:p-8">
            ...
            <tbody>
              {houseEmployees.length > 0 && (
                <tr className="bg-slate-800/80 border-b border-slate-700">
                  <td colSpan={2 + daysInMonth} className="sticky left-0 z-10 p-2 font-bold text-green-400 text-xs md:text-sm pl-4 uppercase tracking-wider backdrop-blur-sm bg-slate-800/80">
                    COLABORADORES DA CASA
                  </td>
                </tr>
              )}
              {houseEmployees.map((emp) => (
                <tr key={emp.id} className="hover:bg-slate-800/40 border-b border-slate-800/50 group">
                  ...
                  <td className={`sticky ${STICKY_NAME_LEFT} z-20 bg-slate-900 group-hover:bg-slate-800 p-2 border-r border-slate-700 font-bold text-slate-300 shadow-lg ${NAME_COL_WIDTH} flex flex-col justify-center`}>
                    <div className="line-clamp-2 leading-tight text-[10px] md:text-xs" title={emp.name}>{emp.name}</div>
                    <button
                      onClick={() => handleDeleteEmployee(emp.id, emp.name)}
                      className="mt-1 text-[8px] text-red-500 hover:text-red-300 uppercase font-black tracking-wider self-start hover:underline"
                    >
                      Excluir
                    </button>
                  </td>
              ...
              {trinoEmployees.length > 0 && (
                <tr className="bg-slate-800/80 border-b border-slate-700">
                  <td colSpan={2 + daysInMonth} className="sticky left-0 z-10 p-2 font-bold text-blue-400 text-xs md:text-sm pl-4 uppercase tracking-wider backdrop-blur-sm bg-slate-800/80">
                    COLABORADORES DA TRINO
                  </td>
                </tr>
              )}
              {trinoEmployees.map((emp) => (
                <tr key={emp.id} className="hover:bg-slate-800/40 border-b border-slate-800/50 group">
                  <td className={`sticky left-0 z-20 bg-slate-900 group-hover:bg-slate-800 p-2 text-center font-mono text-slate-500 ${SHIFT_COL_WIDTH}`}>{emp.shift}</td>
                  <td className={`sticky ${STICKY_NAME_LEFT} z-20 bg-slate-900 group-hover:bg-slate-800 p-2 border-r border-slate-700 font-bold text-slate-300 shadow-lg ${NAME_COL_WIDTH} flex flex-col justify-center`}>
                    <div className="line-clamp-2 leading-tight text-[10px] md:text-xs" title={emp.name}>{emp.name}</div>
        ...
        <div className="mt-6 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2">
          {STATUS_LIST.filter(s => s.code).map(s => (
            <div key={s.code} className="flex items-center gap-2 bg-slate-800/50 p-2 rounded border border-slate-700/50">
              <div className={`w-6 h-6 rounded flex items-center justify-center font-bold text-[10px] ${s.color} ${s.textColor}`}>
                {s.code}
              </div>
